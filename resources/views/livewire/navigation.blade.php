<nav class="bg-gray-800" x-data="{open:false}" x-on:click="open=true">

   {{-- All the nav no responsive --}}
  <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
    ...
    <div class="relative flex items-center justify-between h-16">
      ...
      <!-- Fin Mobile menu button-->

      {{--Logo y Botones al lado del logo --}}
      <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
        ...
        {{--Fin Botones al lado del logo --}}

      </div>
      {{--Finaliza Logo y Botones al lado del logo --}}
    ...
    @auth
          ...
          <div>
            <button x-on:click="open=true" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" aria-haspopup="true">
              ...
              <img class="h-8 w-8 rounded-full"
               src="{{auth()->user()->profile_photo_url}}" alt="pic">
            </button>
          </div>
      ...
      {{-- finaliza notification and Profile dropdown --}}
  ...
  {{-- Finaliza all the nav no responsive --}}
  ...
  <div x-show="open" x-on:click.away="open=false" class="sm:hidden">
    <div class="px-2 pt-2 pb-3 space-y-1">
      ...
      @foreach ($categories as $category)
      <a href="#" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$category->slug}}</a>
      @endforeach

    </div>
  </div>
  {{-- Finaliza responsive mobil --}}
